    ...
    <nav :class="open ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-40 w-64 bg-white border-r border-gray-100 flex flex-col justify-between transition-transform duration-300 transform lg:translate-x-0 no-print shadow-xl lg:shadow-none">
        ...
        <div class="overflow-y-auto">
            ...
            <div class="px-4 py-6 space-y-2">
                ...
                <!-- Menu Umum -->
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('dashboard') ? 'bg-emerald-50 text-emerald-600 font-bold shadow-sm border border-emerald-100' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
                ...
                @if(Auth::user()->role === 'admin')
                    ...
                    <a href="{{ route('admin.umkm.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.umkm.*') ? 'bg-emerald-50 text-emerald-600 font-bold shadow-sm border border-emerald-100' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
                    ...
                    <a href="{{ route('admin.banner.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.banner.*') ? 'bg-emerald-50 text-emerald-600 font-bold shadow-sm border border-emerald-100' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                        Manajemen Banner
                    </a>

                    <a href="{{ route('admin.orders.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.orders.*') ? 'bg-emerald-50 text-emerald-600 font-bold shadow-sm border border-emerald-100' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900 font-medium' }}">
        ...
        <div class="px-4 py-4 border-t border-gray-100 bg-gray-50 shrink-0">
            <div class="flex items-center px-4 py-2 mb-2 rounded-lg bg-white border border-gray-200">
                <div class="w-8 h-8 rounded-full bg-emerald-100 text-emerald-600 flex items-center justify-center font-bold mr-3 shadow-sm shrink-0">
            ...
            <a href="{{ route('profile.edit') }}" class="flex items-center gap-3 px-4 py-2 rounded-lg text-sm text-gray-600 hover:bg-white hover:text-emerald-600 transition font-medium">
            ...
            <form method="POST" action="{{ route('logout') }}" class="w-full">
                @csrf
                <button type="submit" class="w-full flex items-center gap-3 px-4 py-2 mt-1 rounded-lg text-sm text-red-500 hover:bg-red-50 transition font-medium">
